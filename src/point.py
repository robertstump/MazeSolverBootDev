...
class Maze():
    def __init__(self, x1, y1, cell_size_x, cell_size_y, win=None, rows=None, cols=None):
        ...
        self.win = win
        self.cells = []
        self.cell_x_count = rows
        self.cell_y_count = cols

        if self.cell_x_count is not None and self.win is not None:
            if self.cell_x_count * self.size_x > win.width:
                raise ValueError("Total maze width must fit in window width")
        if self.cell_y_count is not None and self.win is not None:
            if self.cell_y_count * self.size_y > win.height:
    ...
    def _create_cells(self):
        ...
        if self.cell_x_count is None: 
            self.cell_x_count = int(inner_width / self.size_x)
        if self.cell_y_count is None:
            self.cell_y_count = int(inner_height / self.size_y)
        last_y = self.y1
        for i in range(self.cell_y_count):
            new_x_list = []
            last_x = self.x1
            for j in range(self.cell_x_count):
                tmp = Cell(last_x, last_y, last_x + self.size_x, last_y + self.size_y, self.win)
                new_x_list.append(tmp)
                last_x += self.size_x
            self.cells.append(new_x_list)     
            last_y += self.size_y
    ...
    def _break_entrance_and_exit(self):
        end_col = self.cell_x_count - 1
        end_row = self.cell_y_count - 1
